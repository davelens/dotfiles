...
# local development.
export RUBY_GC_HEAP_OLDOBJECT_LIMIT_FACTOR="${RUBY_GC_HEAP_OLDOBJECT_LIMIT_FACTOR-1.5}"

# Make sure bootsnap caches to the right location.
export BOOTSNAP_CACHE_DIR="${BOOTSNAP_CACHE_DIR-$XDG_CACHE_HOME}"
...
export SPRING_LOG="${SPRING_LOG-$PWD/log/spring.log}"
export SPRING_TMP_PATH="${SPRING_TMP_PATH-/tmp}"
...
if [[ -n "$BREW_PATH" ]]; then
  export RUBY_CONFIGURE_OPTS="${RUBY_CONFIGURE_OPTS---with-openssl-dir=$BREW_PATH/opt/openssl --with-readline-dir=$BREW_PATH/opt/readline --with-libyaml-dir=$BREW_PATH/opt/libyaml}"
fi
