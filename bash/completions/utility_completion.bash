...
_utility_completions() {
  local input prev subcommand subcommand_completion_path subfolders scripts
  input="${COMP_WORDS[COMP_CWORD]}"
  subcommand="${COMP_WORDS[1]}"
  subcommand_completion_path="$DOTFILES_PATH/bash/completions/${subcommand}_${COMP_WORDS[2]}"

  # Define the root directory for the scripts
  local utilities_root="${DOTFILES_PATH}/bin/utilities"

  # We autoload all bash/completions/*.bash files, so don't add the .bash
  ...
  if [[ $COMP_CWORD -eq 1 ]]; then
    ...
    COMPREPLY=( $(compgen -W "$subfolders" -- "$input") )
    return 0
  fi
  ...
  if [[ -d "$folder_path" && $COMP_CWORD -eq 2 ]]; then
    ...
    COMPREPLY=( $(compgen -W "$scripts" -- "$input") )
    return 0
  fi
