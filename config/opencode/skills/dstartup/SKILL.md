---
name: dstartup
description: Use when starting work in a project that defines startup docs, checklists, or first-step instructions
---

# Startup Context

## When to Use

- Start of a task
- Project defines startup docs, checklists, or first-step rules

Do not use for feature design or durable memory-maintenance decisions.

## Quick Reference

- Run `/caveman` if available.
- Read startup guidance first.
- Follow its checklist items and first-step rules.

## Common Mistakes

- Acting before reading startup guidance
- Skipping `/caveman` in environments that use it for startup behavior
- Skipping startup docs because generic habits seem sufficient
- Applying generic habits when project guidance says otherwise
